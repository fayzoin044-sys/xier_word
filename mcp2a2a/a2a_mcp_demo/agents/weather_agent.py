"""LangChain weather agent backed by the stdio weather MCP server."""
...
根据用户的问题自行判断是否需要调用天气工具。需要具体天气数据时调用工具；
与天气无关的问题可以直接回答。不得编造工具没有返回的天气信息。
"""

WEATHER_MCP_PATH = (
    Path(__file__).resolve().parents[1] / "mcp_servers" / "weather_mcp.py"
)


class WeatherAgent:
    """Discover MCP tools and expose them to a LangChain agent."""
    ...
    async def run(self, user_message: str) -> str:
        """Run one request and let the model decide whether to call a tool."""
